...
public class Templates {
    ...
    static {
        ...
        border = stl.border()
                .setBottomPen(stl.penDashed().setLineWidth(0.3f))
                .setLeftPen(stl.penDouble().setLineColor(Color.BLACK))
                .setTopPen(stl.penDashed().setLineWidth(0.3f))
        ...
        htmlMarkupStyle = DynamicReports.stl.style().setHorizontalAlignment(HorizontalAlignment.LEFT)
                ...
                .setBorder(Templates.border)
                .setPadding(5)
        ;



        generalBorderStyle = stl.style().conditionalStyles(lastRow)
                .setBorder(Templates.border)
                .setPadding(5)
                .setVerticalAlignment(VerticalAlignment.MIDDLE)
                .setHorizontalAlignment(HorizontalAlignment.LEFT);

        wl = Whitelist.simpleText().addTags(
                "a",
                "abbr",
                "acronym",
                "address",
        ...
        return cmp.horizontalList()
                .add(
                        cmp.horizontalList(
                                image,
                                cmp.verticalList(
                                        cmp.text(label).setStyle(bold22CenteredStyle).setHorizontalAlignment(HorizontalAlignment.LEFT)
                                )))
                .newRow()
                .add(cmp.text(DateFormatUtils.format(new Date(), "dd.MM.yyyy")).setStyle(italicStyle).setHorizontalAlignment(HorizontalAlignment.RIGHT)
                )
                .newRow()
                .add(cmp.line())
                .newRow()
    ...
    public static ComponentBuilder<?, ?> createTitleWithSubComponent(String label,String sub) {
        ...
        return cmp.horizontalList()
                ...
                .newRow()
                .add(
                        cmp.verticalList().add(cmp.text(sub).setStyle(Templates.bold12CenteredStyle), cmp.text(DateFormatUtils.format(new Date(), "dd.MM.yyyy")).setStyle(italicStyle).setHorizontalAlignment(HorizontalAlignment.RIGHT)
                        )
                )
                .newRow()
                .add(cmp.line())
    ...
    private static class LastRowCondition extends AbstractComplexExpression<Boolean> {
        ...
        public Boolean evaluate(List<?> list, ReportParameters reportParameters) {
            ...
            int size = (((JRBeanCollectionDataSource)reportParameters.getParameterValue("REPORT_DATA_SOURCE")).getData().size());
