...
public class ValidationProcessor implements IValidationProcessor {
    ...
    @Override
    public <T extends Serializable> List<T> getJSonDataControl(String json, List<Class> lstRequaredClass, Class masterData) throws Exception {
    ...
    // ileride generic method yazma durumu için yazılmıştır sadece bir tane örnektir
    @Override
    public Boolean generalControl(Object value, Method[] methods, HashMap<String, Integer> lenghtMinMax) throws ClassNotFoundException {
        Boolean flag = true;
        ...
        for (Method method : methods) {
            ...
            try {
                if (method.getName().equals("minLenghtControl")) {
                    ...
                    flag = Boolean.parseBoolean(serviceMethod.invoke(base.newInstance(), lenghtMinMax.get("data"),
                            lenghtMinMax.get("lenght")).toString());
